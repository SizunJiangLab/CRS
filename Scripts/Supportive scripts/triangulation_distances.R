...
#' 
#' @param df dataframe containing unique cell id, x position, y position, cell type annotaiton, and region FOV
...
calculate_triangulation_distances <- function(df, 
                                              id, 
                                              x_pos, 
                                              y_pos, 
                                              cell_type, 
  ...
  vtress <- deldir(df[[x_pos]], df[[y_pos]])
...
get_triangulation_distances <- function(df, 
                                        id, 
                                        x_pos, 
                                        y_pos, 
                                        cell_type, 
  ...
  triangulation_distances <- foreach(reg_index = 1:length(unique_regions), 
    ...
    result <- calculate_triangulation_distances(df = subset,
                                                id = id,
                                                x_pos = x_pos,
                                                y_pos = y_pos,
                                                cell_type = cell_type,
...
shuffle_annotations <- function(df, 
                                cell_type, 
                                region, 
                                permutation) {
  unique_regions <- unique(df[[region]])
...
iterate_triangulation_distances <- function(df,
                                            num_iterations = 1000,
                                            id,
                                            x_pos,
                                            y_pos,
  ...
  iterative_triangulation_distances <- foreach(reg_index = 1:length(unique_regions)) %:%
    foreach(iteration_index = 1:num_iterations,
            ...
            .options.snow = opts)%dopar%{
              ...
              df_shuffled <- shuffle_annotations(df = subset,
                                                 cell_type = cell_type,
                                                 region = region,
                                                 permutation = iteration_index)
              
              results <- get_triangulation_distances(df = df_shuffled,
                                                     id = id,
                                                     x_pos = x_pos,
                                                     y_pos = y_pos,
